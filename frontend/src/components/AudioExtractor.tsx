...
export const AudioExtractor: React.FC = () => {
  ...
  useEffect(() => {
    fetch('http://192.168.1.198:4001/api/audio/formats')
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleExtractAudio = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.198:4001/api/audio/extract', {
        method: 'POST',
  ...
  const pollJobStatus = async (jobId: number) => {
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`http://192.168.1.198:4001/api/audio/jobs/${jobId}`);
        const data = await response.json();
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.1.198:4001/api/audio/jobs/${currentJob.id}/download`
      );
