...
export default function JobsHistory() {
  ...
  const fetchJobs = async () => {
    try {
      const res = await fetch('http://192.168.1.198:4001/api/jobs?limit=100');
  ...
  const handleDelete = async (jobId: number) => {
    ...
    try {
      const res = await fetch(`http://192.168.1.198:4001/api/jobs/${jobId}`, {
        method: 'DELETE',
      });
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                {job.thumbnailPath && (
                  <div className="flex-shrink-0">
                    <img
                      src={`http://192.168.1.198:4001/api/jobs/${job.id}/thumbnail`}
                ...
                <div className="flex flex-col space-y-2 ml-4">
                  {job.status === 'completed' && job.outputFile && (
                    <a
                      href={`http://192.168.1.198:4001/api/jobs/${job.id}/download`}
