...
export const VideoTrimmer: React.FC = () => {
  ...
  const handleTrim = async () => {
    ...
    try {
      const response = await fetch('http://0.0.0.0:4001/api/trim', {
        method: 'POST',
  ...
  const pollJobStatus = async (jobId: number) => {
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`http://0.0.0.0:4001/api/trim/jobs/${jobId}`);
        const data = await response.json();
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(
        `http://0.0.0.0:4001/api/trim/jobs/${currentJob.id}/download`
      );
