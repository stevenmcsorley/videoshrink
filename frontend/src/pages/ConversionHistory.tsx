...
export default function ConversionHistory() {
  ...
  const fetchJobs = async () => {
    try {
      const url =
        filter === 'all'
          ? 'http://0.0.0.0:4001/api/convert/jobs'
          : `http://0.0.0.0:4001/api/convert/jobs?status=${filter}`;
  ...
  const handleDownload = async (jobId: number) => {
    window.open(`http://0.0.0.0:4001/api/convert/jobs/${jobId}/download`, '_blank');
  };
  ...
  const handleDelete = async (jobId: number) => {
    ...
    try {
      await fetch(`http://0.0.0.0:4001/api/convert/jobs/${jobId}`, {
        method: 'DELETE',
      });
      fetchJobs();
    } catch (error) {
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                {job.thumbnailPath && (
                  <div className="flex-shrink-0">
                    <img
                      src={`http://0.0.0.0:4001/api/convert/jobs/${job.id}/thumbnail`}
